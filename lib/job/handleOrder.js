  ...
  pushToOrder() {
    ...
    const pubToRedis = (next) => {
      ...
      redisConnections('master')
        .getConnection()
        .publish('newFeeds', JSON.stringify([JSON.stringify(jsonCompress.compress(obj, templateWhole))]), next)

      if(this.timesPush === 0) {
        redisConnections('master')
          .getConnection()
          .lpush('feedsFacebook', [JSON.stringify(obj)], (err, res) => {
            if(err) {
