...
module.exports = (req, res) => {
  ...
  const userAuten = config.lending.environment ? config.lending.user.live : config.lending.user.test;
  ...
  const getBankCodes = (next) => {
    const options = {
      ...
      headers: {
        'Authorization': 'Basic ' + new Buffer(userAuten + ':' + passAuthen).toString('base64')
      },
